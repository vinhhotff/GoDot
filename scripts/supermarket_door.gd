...
@export var prompt_message: String = "Cửa siêu thị khóa (Hãy nói chuyện với đồng nghiệp trước)"
...
func interact(player_node) -> void:
	...
	if level.dialogue_triggered and level.current_step >= level.dialog_steps.size():
		...
		level.show_dialog(" Aaron: 'Mình nên lại gần nói chuyện với đồng nghiệp ca trước đang đứng trước cửa kia đã.'")
